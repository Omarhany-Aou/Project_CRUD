...
if (isset($_POST['save'])) {
    ...
    if ($_POST['save'] == "save"){
    $save_sql = "INSERT INTO `users`(`name`, `email`, `password`, `mobile`) VALUES ('$name','$email','$mobile', '$password')";
    }else{
        $id=$_POST['id'];
        $save_sql = "UPDATE `users` SET `name` ='$name', `email`='$name', `password`='$password', `mobile`='$mobile' WHERE `id` =$id ";
    }
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex p-2 justify-content-evenly mb-2">
                                    <i onclick="confirm_delete(<?php echo $user['id'];?>);" class="text-danger" class data-feather="trash-2"></i>
